calibration_document = """sdpgz3five4seven6fiveh
876mbxbrntsfm
...
def extract_calibration_value(line):
    new_line = line.replace("one", "one1one").replace("two", "two2two").replace("three", "three3three").replace("four", "four4four").replace("five", "five5five").replace("six", "six6six").replace("seven", "seven7seven").replace("eight", "eight8eight").replace("nine", "nine9nine")
    digits = [x for x in new_line if x.isdigit()]
    return int(digits[0])*10 + int(digits[-1])

def sum_calibration_values(document):
    calibration_values = [extract_calibration_value(line) for line in document.split("\n")]
